...
class Operation:
    def __init__(
        ...
        webhook: str = None,
        id: int = None,
        members: List[Union["Operator", Dict[str, str]]] = None,
    ):
        self._name = name
        ...
        if members is not None:
            if isinstance(members, list):
                self._members = [
                    Operator(username=x) if isinstance(x, str) else x for x in members
                ]
            else:
...
class Mythic:
        ...
        try:
            ...
            if "channel" in json_data:
                if "callback" in json_data["channel"]:
                    ...
                    await json_print(EventMessage(**json_data))
                elif "event" in json_data["channel"]:
                    await json_print(EventMessage(**json_data))
            elif "chunks_received" in json_data:
                print("printing filemeta info")
                await json_print(FileMeta(**json_data))
            elif "build_phase" in json_data:
                print("printing payload info")
                await json_print(Payload(**json_data))
            elif "agent_task_id" in json_data:
                print("printing task data")
                await json_print(Task(**json_data))
            elif "response" in json_data:
                print("printing response data")
                await json_print(Response(**json_data))
            elif "realm" in json_data:
                print("printing credential data")
                await json_print(Credential(**json_data))
            elif "level" in json_data:
                print("printing event message data")
                await json_print(EventMessage(**json_data))
            else:
                print("not casting")
                    ...
                    try:
                        ...
                        if msg.data != "":
                            task = asyncio.get_event_loop().create_task(
                                callback_function(self, data=msg.data)
                            )
    ...
    async def create_operator(self, username: str, password: str) -> MythicResponse:
        url = "{}{}:{}/api/v{}/operators".format(
            self._http, self.server_ip, self._server_port, self._server_api_version
        )
    ...
    async def download_file(self, file_id) -> bytes:
        ...
        :param file_id:
            if using FileMeta class, the following must be set:
                agent_file_id
        :return:
        """
        str_id = ""
        if isinstance(file_id, FileMeta):
            str_id = file_id.agent_file_id
    ...
    async def create_task(
        ...
        if resp.response_code == 200 and resp.status == "success":
            ...
            print("task successfully submitted now about to wait for required response")
    ...
    async def gather_task_responses(self, task_id, timeout=None):
        """
